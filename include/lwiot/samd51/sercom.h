...
// the clock-sourcing functions just compile to nothing on SAMD21.
		...
		public:
			...
			void initFrame(SercomUartCharSize charSize, SercomDataOrder dataOrder, SercomParityMode parityMode,
			...
			SercomDataOrder getDataOrderSPI();
			...
			SercomClockSource getClockSource(void)
			{
				return clockSource;
			};

			uint32_t getFreqRef(void)
			{
				return freqRef;
			};
#else
