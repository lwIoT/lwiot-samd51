...
namespace lwiot
{
	namespace samd51
	{
		...
		SERCOM::SERCOM(Sercom *s) : sercom(s), freqRef(SERCOM_FREQ_REF)
		{
...
*/
		...
		void SERCOM::initFrame(SercomUartCharSize charSize, SercomDataOrder dataOrder, SercomParityMode parityMode,
...
*/
		void
		SERCOM::initSPI(SercomSpiTXPad mosi, SercomRXPad miso, SercomSpiCharSize charSize, SercomDataOrder dataOrder)
		{
			resetSPI();
			initClockNVIC();
...
//  sercom->I2CM.INTENSET.reg = SERCOM_I2CM_INTENSET_MB | SERCOM_I2CM_INTENSET_SB | SERCOM_I2CM_INTENSET_ERROR ;
		...
		void SERCOM::prepareNackBitWIRE()
		{
			if(isMasterWIRE()) {
				// Send a NACK
		...
		bool SERCOM::startTransmissionWIRE(uint8_t address, SercomWireReadWriteFlag flag)
		{
			...
			return sercom->I2CM.STATUS.bit.RXNACK == 0;
		}
		...
		bool SERCOM::isRXNackReceivedWIRE()
		{
		...
		int8_t SERCOM::getSercomIndex()
		{
			for(auto i = 0U; i < (sizeof(sercomData) / sizeof(sercomData[0])); i++) {
				if(sercom == sercomData[i].sercomPtr)
					return i;
			}
...
// so user code doesn't require a lot of conditional situations.
		void SERCOM::setClockSource(int8_t idx, SercomClockSource src, bool core)
		{
			...
			if(src == SERCOM_CLOCK_SOURCE_NO_CHANGE)
				return;
